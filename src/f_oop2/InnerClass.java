...
class Outer{
	...
	class InstanceInner{
		...
		void method(){
			...
			System.out.println(Outer.this.var); // 가장 밖의 클래스에 있는 변수. 클래스명.this.변수이름.
		}
	}
...
//		int inner1 = instanceVar; //컴파일 에러 발생
		int inner2 = classVar;
	...
	void method(){
		...
		//지역 클래스
		class LocalInner{
			//지역 클래스에서는 인스턴스 멤버와 클래스 멤버 모두 사용 가능하고, final 이 붙은 지역 변수도 사용 가능
			int inner1 = instanceVar;
			int inner2 = classVar;
//			int ineer3 = localVar1; //final 이 붙지 않은 지역변수는 사용 불가
	...
	void test1(){
		...
		//클래스 맴버는 클래스를 통해서 바로 접근할 수 있는 것과 같이 스태틱 클래스는 클래스를 통해서 바로 접근할 수 있다.
