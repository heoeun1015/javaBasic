...
/*
		...
		*/
		...
		- 오버라이딩: 부모 메서드지만 이름만 같고 수행 내용이 다를 때,
				    리턴 타입/메서드명/파라미터는 같게 한 후 구현부의 내용을 다르게 쓸 때 사용한다.
		- 필요성: 부모 메서드와 다른 기능을 사용할 때 필요하다.
		
		
		
		
		[7-4] 다음 중 오버라이딩의 조건으로 옳지 않은 것은? (c)
				a. 조상의 메서드와 이름이 같아야 한다.
				b. 매개변수의 수와 타입이 모두 같아야 한다.
				c. 접근 제어자는 조상의 메서드보다 좁은 범위로만 변경할 수 있다. //넓은 범위
		...
		*/
		...
		/*
		 ...
		 - 조상 클래스의 생성자는 그대로 사용할 수 없기 때문에 따로 호출해 초기화 해주어야 한다.
		     조상 클래스에서 정의된 인스턴스 변수들이 초기화되도록 하기 위해서.
		...
		class Child extends Parent {
			...
			Child(int x) { //(2) //뒤에 슈퍼가 붙어 있다. 슈퍼는 둘 중 하나만 들어 있음. //this가 없는 생성자에 super가 들어간다. 
				this.x = x;
			}
		}
		class Exercise7_7 {
		...
		*/
		...
		[7-9] 다음 중 제어자 final을 붙일 수 있는 대상과 붙였을 때 그 의미를 적은 것이다.	옳지 않은 것은? ( c )
			a. 지역변수 - 값을 변경할 수 없다.
			b. 클래스 - 상속을 통해 클래스에 새로운 멤버를 추가할 수 없다.
			c. 메서드 - 오버로딩을 할 수 없다. // 오버라이딩을 할 수 없다.
			d. 멤버변수 - 값을 변경할 수 없다.
			
			//상수 = 값을 변경할 수 없다. 클래스에서는 상속을 받아서 그 의미를 확장할 수 없다.
		*/
		...
		*/
		...
		d. AirUnit au = ac;
		e. t = (Tank)u; // 조상 타입의 인스턴스를 자손타입으로 형변환 할 수 없다. 다운캐스팅.
		f. GroundUnit gu = t;
		*/
...
//		}
		...
		*/
		...
		돈(money)과 장바구니(cart)를 가지고 있다. 제품을 구입하는 기능의 buy메서드와 장바구니에구입한 물건을
		추가하는 add메서드, 구입한 물건의 목록과 사용금액, 그리고 남은 금액을 출력하는 summary메서드를 완성하시오.
