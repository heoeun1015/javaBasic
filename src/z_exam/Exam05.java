...
public class Exam05 {
	...
	public static void main(String[] args) {
		...
		b. int[] arr = {1,2,3,};
		c. int[] arr = new int[5];
		d. int[] arr = new int[5]{1,2,3,4,5};	: 두 번째 대괄호에 숫자 넣으면 안 됨.
		e. int arr[5];							: 배열 선언. 크기 지정 불가. 대입 연산자 후 중괄호에 넣어줘야 함.
		f. int[] arr[] = new int[3][];			
		*/
	...
	/*	[5-5] 다음은 1과 9사이의 중복되지 않은 숫자로 이루어진 3자리 숫자를 만들어내는 프
		...
		(2) System.arraycopy(ballArr, 1, ball3, 1, 3);
			또는
			for(int i = 0; i < ball3.length; i++){
				ball3[i] = vallArr[i];
			}
...
/*
		...
		**
		...
		1,0 > 0,2	1,1 > 1,2   1,2 > 2,2   1,3 > 3,2   1,4 > 4,2
		2,0 > 0,1	2,1 > 1,1   2,2 > 2,1   2,3 > 3,1   2,4 > 4,1
		...
		class Exercise5_13 {
			public static void main(String args[]) {
				...
				for(int i=0;i<words.length;i++) {
					...
					(1) 랜덤한 인덱스를 발생시켜서 0번 인덱스와 자리를 바꾼다.
