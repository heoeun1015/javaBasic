...
public class EtcOperator {
	...
	public static void main(String[] args) {
		 ...
		 * - &(AND)		: 피연산자 중 양 쪽이 모두 1이어야만 1을, 그 외는 0을 결과로 얻는다.
		 * - ^(XOR)		: 피연산자의 값이 서로 다를 때만 1을, 같을 때는 0을 결과로 얻는다.
		 * - ~(비트전환)	: 피연산자를 0은 1로, 1은 0으로 바꾼다. (피연산자 1개)
		 * - <<(쉬프트)	: 피연산자의 각 자리를 왼쪽으로 이동시킨다. 빈칸은 0으로 채운다.
		 * - >>(쉬프트)	: 피연산자의 각 자리를 오른쪽으로 이동시킨다. 빈칸은 0앞자리와 동일한 값으로 채운다.
		 ...
		 * 2  1
		 * 1  0
		 * 10 = 1010(2진수)(좌측하단부터 우측상단으로 올라간 값)
		 *
		 * - 2진수 → 10진수:  각 자리에 1, 2, 4, 8, 16... 를 곱한 값을 더한다.
		 * 1 0 1 0 (맨 오른쪽에서부터 1씩 증가)
		 * 8 4 2 1
		 * 8+0+2+0 = 10
		...
		//>>	: 00000101
		System.out.println(10 >> 1);
		//산술연산자보다 속도는 빠른데 자주 사용하지는 않는다.
		
		
		
		int x = 10;
		int y = 20;
		int result = ( x < y ) ? x : y; //참이면 x, 거짓이면 y가 result에 저장된다.
		...
		int b = 21;
		int bigger = (a < b) ? b : a;
		System.out.println("큰 숫자는 " + bigger + "입니다.");
		...
		//★ 문제3. 변수에 저장된 주민등록번호 뒷자리의 첫 번째 숫자가 1이나 3이면 남자를,
		//2나 4면 여자를 출력해주세요.
		//그 외의 숫자를 입력하면 "잘못 입력하셨습니다."를 출력해주세요.
		int d = 2;
		String gender2 = d == 1 || d == 3 ? "남자" :
			d == 2 || d == 4 ? "여자" : "잘못 입력하셨습니다.";
