...
public class Score {
	...
	public static void main(String[] args) {
		...
		double[][] grade = new double[students.length][subjects.length + 3];
		
		
		int[] sum = new int[subjects.length];
		int[] avg = new int[subjects.length];
		
		
		
		for(int i = 1; i < subjects.length; i++){
			for(int j = 1; j < subjects.length; j++){
				subjects[i] = (int)(Math.random() * 100) + 1;
			}
		}System.out.println(Arrays.toString(subjects[i]));
		...
		*/
		...
		for(int i = 0; i < scores.length; i++){
			...
			scores[i][scores[i].length - 1]
					= (int)((double)scores[i][scores[i].length - 2]	/ subjects.length * 100 + 0.5) / 100d; //평균
		}
